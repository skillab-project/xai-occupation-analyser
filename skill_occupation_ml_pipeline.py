...
def train_and_evaluate_model(file_path: str = "datasets/all_occupation4d_skills.csv", min_category_samples: int = 50, test_size: float = 0.1):
    """
    Trains a OneVsRest RandomForest model on occupation/job data, evaluates performance, and generates SHAP analysis.

    Parameters:
        file_path (str): Path to the input CSV file.
        min_category_samples (int): Minimum number of samples per category to be considered for model training.
        test_size (float): Proportion of data to be used for testing.

    Returns:
        str: Path to the saved model performance metrics file.
    """
    ...
    # Data filtering: Keep only categories with at least min_category_samples
    ...
    # Train-test split
    X_train, X_test, y_train, y_test = train_test_split(X_filtered, y_filtered, test_size=test_size, random_state=42, stratify=y_filtered)
    ...
    # model_file = os.path.join(output_folder_models, "ovr_model_features.pkl")
    # with open(model_file, "wb") as model_file:
    #     pickle.dump(ovr_model, model_file)
    
    # Calculate performance metrics
    y_pred_proba = ovr_model.predict_proba(X_test)
    roc_auc_scores, precision_scores, recall_scores, f1_scores = {}, {}, {}, {}
    ...
    print(f"Training and evaluation completed. Performance metrics saved at {models_performance_file}")
    return str(models_performance_file)
